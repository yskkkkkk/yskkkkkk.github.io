...
거기서 이야기가 정보이론의 Claude Shannon으로 넘어가고, `daemon`·`fork`·`grep`·`mutex`·`semaphore` 같은 용어들의 유래로 이어지다가, 결국 Git에 도착했습니다. "그러고 보니 이건 왜 이름이 git이지?" 하는 생각이 들었거든요.

그 질문 하나에서 출발해서 Git의 내부 구조를 뜯어보고, 나중에는 rebase 실습까지 하게 됐습니다. 잊어버리기 전에 정리해둡니다.
...
여담이지만 한 사람이 운영체제와 버전관리 시스템을 둘 다 만들어서 각각 업계 표준으로 굳혀버렸다는 게 새삼 대단하게 느껴집니다.
...
가계도에 비유하니 이해가 쉬웠습니다. 각 사람은 '내 아버지가 누구다'만 알지 '내 자식이 몇이다'는 저장하지 않잖아요. 그런데 모든 사람의 아버지 정보만 모으면 거꾸로 추적해서 완전한 가계도를 그릴 수 있습니다. Git이 정확히 이 원리예요. 손 빠른 제작자가 매번 "당신 아버지가 누구예요?"를 물어 그래프를 새로 그려내는 셈이거든요.
...
`boolean`의 어원 하나가 궁금했을 뿐인데 하루가 이렇게 다 갔네요.

매일 쓰던 `git rebase`, `git merge`가 내부에서 정확히 무슨 일을 하는지 알고 나니, 충돌이 왜 나는지, 언제 위험한지가 훨씬 선명해졌습니다. 특히 '충돌 없이 성공했다'가 '코드가 정상이다'와 같은 말이 아니라는 것, 그리고 분기는 상태고 충돌은 사건이라는 것. 이 두 가지는 앞으로도 오래 써먹을 것 같아요.
