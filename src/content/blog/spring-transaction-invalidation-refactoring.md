...
제가 정확히 이해가 안 가는 부분이 있어, 아예 프로젝트 설정 파일까지 전부 에이전트에게 넘겨주고 분석을 부탁했습니다.

그런데 코드를 빤히 보던 에이전트가 조심스럽게 한마디 하더군요.

"이거... 트랜잭션 작동 안 하고 있는 것 같은데요?"

에이전트에게 뒤통수를 맞은 기분이었습니다.

설마 하는 마음에 제가 직접 설정 파일과 코드를 한 번 더 더블 체크해 보았습니다.

결과는 진짜였습니다. 스프링 설정 파일에 있어야 할 `<tx:annotation-driven>` 설정이 완벽하게 누락되어 있었던 겁니다.
