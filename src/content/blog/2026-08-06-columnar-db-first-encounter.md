...
## 먼저 필요한 건 과거를 남기는 일입니다

정리하고 넘어가면, 기획자분 질문에 대한 답은 지나간 상태를 어딘가 남겨두는 것부터입니다.

방법은 몇 가지 있습니다. 집계 결과만 매일 쌓아두는 방법도 있고, 값이 바뀔 때만 이력으로 기록하는 방법도 있고요. 그중 가장 단순한 건 스냅샷입니다. 매일 특정 시점의 상태를 그대로 찍어서 별도 테이블에 쌓아두는 거요.

그러면 7월 1일 기준으로 회원 등급이 무엇이었는지를 나중에도 그대로 꺼내볼 수 있습니다. 원본이 몇 번을 덮어써지든 상관없이요.
...
제가 이해한 걸 그대로 되짚어 말해봤습니다. "행 지향은 회원 한 명의 정보를 한곳에 모아두는 거라면, 열 지향은 모든 회원의 이름을 한 데 모아두고, 모든 회원의 전화번호를 또 한 데 모아두고, 이런 식인 거죠?" 맞다는 답을 듣고서야 그림이 완전히 그려졌습니다.

알고리즘 문제 풀 때나 보던 저장 방식이 실제 DB 구조로 쓰이고 있다는 것도 재밌었고요. 듣자마자 "대용량이면 무조건 이게 낫겠는데?" 싶었습니다.

## 숫자로 다시 확인해봤습니다

이해한 게 맞는지 구체적인 숫자로 한 번 더 짚어보고 싶었습니다. 회원 10만 명, 컬럼 10개짜리 표에서 조건에 맞는 3만 명만 골라 이름과 나이, 두 컬럼만 보고 싶다고 가정해봤습니다.

행 지향이면 후보가 될 만한 블록들을 뒤지면서, 그 안에 있는 사람마다 10개 컬럼을 통째로 메모리에 올려야 합니다. 그러고 나서야 이름과 나이만 골라낼 수 있고요. 필요한 건 2개 컬럼인데 실제로 읽는 건 10개 컬럼 전체입니다.

열 지향이면 나이 컬럼 파일 하나만 열어서 조건에 맞는 3만 명의 위치를 찾고, 이름 컬럼 파일에서 그 위치에 해당하는 값만 꺼내 옵니다. 처음부터 끝까지 두 파일 바깥으로 나갈 일이 없습니다.

숫자로 그려보니 왜 이렇게 차이가 나는지 확실히 감이 왔습니다. 10개 컬럼 중 2개만 쓰는 상황이면, 나머지 8개를 읽는 시간은 전부 낭비였던 거니까요.

쿼리 문법 자체는 SELECT, WHERE 다 똑같이 쓴다고 하더라고요. 다만 날짜 조건을 다루는 감각이 다르다는 이야기를 들었는데, 그 부분까지는 아직 제대로 못 알아봤습니다.
...
시간이 없어서 여기까지만 알아봤습니다. 그래도 간만에 듣고 나서 눈이 번쩍 뜨이는 문제 해결법이었습니다.
